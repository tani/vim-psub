...
C<foo1> and C<foo2> in a line, you can use C<:S/foo{1,2}/foo{2,1}/g> to do.

The command C<:PSub> accepts the same arguments as C<:s> does. For
example, C<:P/foo/bar/g> is equivalent to C<:P/foo/bar/g>. The only
difference is that C<:S> expands a pattern in parallel by C<{}>.

The command C<:PSub> also accepts a range. For example, C<< :'<,'>P/foo/bar/g >>
